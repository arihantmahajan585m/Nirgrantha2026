...
async function maybeLoadMockBackend(
  ...
  if (!force && import.meta.env.VITE_USE_MOCK !== "true") {
    return null;
  }
...
export async function createActorWithConfig(
  ...
  try {
    ...
    const fallback = await maybeLoadMockBackend(true);
    if (fallback) {
      console.warn(
        "Falling back to the local persistent backend because the configured backend could not be created.",
        error,
      );
      return fallback;
    }
